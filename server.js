...
const firebaseConfig = {
  apiKey: 'AIzaSyBYN9ycUwz9mQ7OOeQFVeH2N5eVW0r4KfQ',
  authDomain: 'prova-6c5b2.firebaseapp.com',
  projectId: 'prova-6c5b2',
  storageBucket: 'prova-6c5b2.appspot.com',
  messagingSenderId: '223123705989',
  appId: '1:223123705989:web:cf357e11c10da3fe5c59ec',
};
...
// middlewares
//to make the static folder the server should look at and read the java and css files
app.use(express.static('public')); // after this code you can see the webpage on localhost:3000
app.use(express.json()); // emables the data form sharing with the server
...
app.post('/signup', (req, res) => {
  ...
  // form validation
  if (name.length < 3) {
    res.json({ alert: 'name must be 3 letters long at least' });
    ...
    res.json({ alert: 'you must agree our terms and conditions' });
  } else {
...
app.post('/seller', (req, res) => {
  ...
  if (
    ...
    return res.json({ alert: 'some information is missing' });
  } else {
...
app.post('/add-product', (req, res) => {
  ...
  if (!draft) {
    if (!name.length) {
      ...
      res.json({ alert: 'short des must be 80 letter long' });
    } else if (!price.length || !Number(price)) {
      res.json({ alert: 'enter a valid price' });
...
app.post('/add-review', (req, res) => {
  ...
  setDoc(doc(reviews, docName), req.body)
    ...
    .catch((err) => {
      console.log(err);
      res.json({ alert: 'there is some error' });
    });
});
